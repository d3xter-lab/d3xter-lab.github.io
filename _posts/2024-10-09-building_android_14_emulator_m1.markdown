...
일단 AVD를 위한 AOSP 빌드하는 전체 코드를 위와 같습니다. 그런데 이제 에러들이 툭툭 나오게 될거에요. 우선 이전에 언급한 메모리 부분. 32GB 이상을 사용하시는 분은 아마 별 문제 없을건데 저같은 16GB 메모리의 경우, 메모리 문제로 빌드에 실패하는 경우가 있습니다.
